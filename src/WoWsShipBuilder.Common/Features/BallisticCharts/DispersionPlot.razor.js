...
function drawDispersionPlot(id, drawingData, scaling, fusoReference, text) {
    ...
    if (canvas === null) {
        console.log(`Cannot find canvas for id ${id}`);
        return;
    }
    ...
    const textSize = ctx.measureText(
        [topText, bottomText, leftText, rightText].reduce(
            (oldLongestString, nextString) => oldLongestString.length > nextString.length ? oldLongestString : nextString,
            ""
        ));
